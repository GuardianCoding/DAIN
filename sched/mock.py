...
from sched.cost import fits, predict_tok_s, node_footprint_mb
...
MOCK_PROFILES = [
    NodeProfile(
        id="gpu-01", host="192.168.50.10", cpu="Intel Core i7", cores=12,
        ram_total_mb=64 * 1024, ram_free_mb=46 * 1024,
        gpu="NVIDIA GeForce RTX 5070 Ti", vram_total_mb=16 * 1024,
        backend="cuda", mem_bandwidth_gbs=650.0,
        tg_tok_s=45.0, pp_tok_s=180.0, rtt_ms=0.3, state="idle",
    ),
    NodeProfile(
        id="office-01", host="192.168.50.11", cpu="Intel Core i7 vPro", cores=8,
        ram_total_mb=8 * 1024, ram_free_mb=6 * 1024,
        gpu="Intel integrated graphics", vram_total_mb=0,
        backend="cpu", mem_bandwidth_gbs=38.0,
        tg_tok_s=9.0, pp_tok_s=65.0, rtt_ms=0.4, state="idle",
    ),
    NodeProfile(
        id="office-02", host="192.168.50.12", cpu="Intel Core i7 vPro", cores=8,
        ram_total_mb=8 * 1024, ram_free_mb=6 * 1024,
        gpu="Intel integrated graphics", vram_total_mb=0,
        backend="cpu", mem_bandwidth_gbs=19.0,   # single DIMM — half the bandwidth
        tg_tok_s=4.5, pp_tok_s=33.0, rtt_ms=0.4, state="idle",
    ),
    NodeProfile(
        id="mac-01", host="192.168.50.13", cpu="Apple M5 Pro", cores=12,
        ram_total_mb=24 * 1024, ram_free_mb=18 * 1024,
        gpu="Apple M5 Pro integrated GPU", vram_total_mb=24 * 1024,
        backend="metal", mem_bandwidth_gbs=270.0,
        tg_tok_s=22.0, pp_tok_s=110.0, rtt_ms=0.5, state="idle",
    ),
]

MOCK_METRICS = [
    NodeMetrics(node_id="gpu-01", timestamp=0.0, cpu_percent=18.0,
                ram_free_mb=48 * 1024, gpu_percent=25.0, vram_free_mb=12 * 1024, jobs_running=0),
    NodeMetrics(node_id="office-01", timestamp=0.0, cpu_percent=22.0,
                ram_free_mb=6 * 1024, gpu_percent=None, vram_free_mb=None, jobs_running=0),
    NodeMetrics(node_id="office-02", timestamp=0.0, cpu_percent=20.0,
                ram_free_mb=6 * 1024, gpu_percent=None, vram_free_mb=None, jobs_running=0),
    NodeMetrics(node_id="mac-01", timestamp=0.0, cpu_percent=15.0,
                ram_free_mb=14 * 1024, gpu_percent=10.0, vram_free_mb=None, jobs_running=0),
]
